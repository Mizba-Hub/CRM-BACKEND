...
const formatTicketResponse = (ticket) => {
  const dealLeadDetails = ticket.Deal?.associatedLead;
  const companyLeadDetails = ticket.Company?.Lead;
  const recipientEmail =
    dealLeadDetails?.email ?? companyLeadDetails?.email ?? null;
  ...
  return {
    ...
    deal: ticket.Deal
      ? {
          id: ticket.Deal.id,
          name: ticket.Deal.dealName,
          phoneNumber: dealLeadDetails?.phoneNumber ?? null,
          city: dealLeadDetails?.city ?? null,
          email: dealLeadDetails?.email ?? null,
          lead: formatLeadDetails(dealLeadDetails),
        }
      : ticket.dealId
      ? { id: ticket.dealId, name: null, phoneNumber: null, city: null }
      : null,
    company: ticket.Company
      ? {
          ...
          phoneNumber: ticket.Company.phoneNumber,
          city: ticket.Company.city,
          email: companyLeadDetails?.email ?? null,
          lead: formatLeadDetails(companyLeadDetails),
        }
      : ticket.companyId
      ? { id: ticket.companyId, name: null, phoneNumber: null, city: null, email: null, lead: null }
      : null,
    createdAt: ticket.createdAt,
  };
};
